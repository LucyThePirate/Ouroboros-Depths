class_name SymmetricShadowcasting extends Node

# CREDIT TO: https://github.com/ghostsoft/SymmetricShadowcastingGDScript/blob/main/symmetric_shadowcasting.gd
...
func is_occluding(x: int, y: int) -> bool:
	var checking_tile = Vector2i(x, y)
...
func reveal_tile(x: int, y: int):
	var tile = Vector2i(x, y)
	Global.visible_tiles[tile] = 1


func computeFov(
	origin: Vector2i,
	fov_distance: int = 7,
	mark_visible: Callable = reveal_tile,
	is_blocking: Callable = is_occluding
):
...
func is_symmetric(row: Row, tile) -> bool:
	...
	var col = tile.y
	return (
		col >= row.depth * row.start_slope.toFloat() and col <= row.depth * row.end_slope.toFloat()
	)
